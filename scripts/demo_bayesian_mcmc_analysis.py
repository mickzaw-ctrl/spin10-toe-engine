...
MCMC (Markov Chain Monte Carlo) of multi-dimensional parameter space
of fundamental ToE parameters using the emcee library.

Determin parameters Best-Fit dla M_SUSY, alpha_attractor oraz Causal Fraction CF,
...
def run_demo():
    ...
    print(f"   - Granica tensors r:            < {likelihood.exp_data['r_upper_limit']}             [BICEP/Keck]")
    print(f"   - Proton lifetime tau_p:      > {likelihood.exp_data['tau_p_lower_limit']:.1e} lat      [Super-Kamiokande]")
    ...
    print(f"   Number errorzacych: n_walkers = {n_walkers}")
    print(f"   Number steps:     n_steps   = {n_steps} (Odrzucenie relaksacji burn-in: {burnin})")
    print(f"   Suma ewaluacji w pelni multi-dimensional space: {n_walkers * n_steps:,} wywolan...")
    ...
    # 3. Results estymacji parameterow Best-Fit
    print(f"\n3. ROZKLADY A POSTERIORI I ESTYMACJA PARAMETROW (Best-Fit & Percentyle)")
    print(f"   {'Parameter ToE':<22} | {'MAP Best-Fit':<14} | {'Mediana (50%)':<14} | {'Przedzial 68% (1-sigma)':<25}")
    print("   " + "-"*80)
    ...
    print(f"   - Proton lifetime tau_p:      tau = {bobs['tau_p']:.2e} lat (W Hyper-K zasiegu 1e35)")
    print(f"   - Mass gluina Split-SUSY:        m_g = {bobs['m_gluino']/1000:.2f} TeV      (Poza zasiegiem LHC)")
    print(f"   - Uprofitana asymmetry barionowa:  eta = {bobs['eta_B']:.2e}   (Idealna zgodnosc)")
    
    # 5. Tworzenie i saveywanie plotu Posterior (jesli matplotlib dostepny)
    try:
        ...
        print(f"\n   (Library 'matplotlib' niedostepna --- pominieto generowanie file PNG).")

    print("\n   >>> Module 'MultiExperimentLikelihood' gotowy do pelnej integracji z repozytorium! <<<")
    print("="*75)
