...
Wykonuje:
  1. Baseline matching call for M_SUSY = 5 TeV (consistent with specification).
  2. Parametric Sweep over different Split-SUSY breaking scales
     (1 TeV -> 50 TeV), showing the impact on the M_GUT scale and emergent Weinberg angle.
  3. Generating high-quality publication-ready plot in PNG format
     showing the evolution of inverse couplings (alpha_i^-1) as a function of energy scale mu.
...
def run_suite():
    ...
    # -------------------------------------------------------------------------
    # 1. BASELINE CALL (M_SUSY = 5 TeV)
    # -------------------------------------------------------------------------
    print("\n1. CALL BASELINE DLA M_SUSY = 5 TeV (5000 GeV)")
    ...
    print(f"   Numerical computations completed in {exec_time:.2f} s.")
    print(f"   >>> Scale Unifikacji:          M_GUT      = {analysis['M_GUT_GeV']:.2e} GeV")
    print(f"   >>> Zunifikowane coupling:   alpha_GUT  = {analysis['alpha_GUT']:.4f}  (alpha_GUT^-1 = {analysis['alpha_GUT_inv']:.2f})")
    ...
    print("   Badamy, jak zmiana mass superpartnerow impacta na warunek idealnej unification...")
    ...
    try:
        ...
        # Determinmy trajektorie dla M_SUSY = 5 TeV
        mu_vals = np.exp(t_vals)
        # Coupling alpha_1^-1, alpha_2^-1, alpha_3^-1
        ...
        plt.title(r"Wielka Unification Sprzezen Gauge w Spin(10) ToE ze Split-SUSY", fontsize=14, pad=15)
        plt.xlabel(r"Logarytm scale energy $\log_{10}(\mu/\text{GeV})$", fontsize=12)
        plt.ylabel(r"Inverse constant coupling $\alpha_i^{-1}(\mu)$", fontsize=12)
        ...
        print(f"   Wygenerowano i saveano plot: '{os.path.basename(out_path)}' (rozdzielczosc 250 DPI).")
        print("   Plot idealnie obrazuje zbieznosc sil przyrody w jednym punkcie quantum gravity.")
    except Exception as e:
        print(f"\n   (Error generowania plotu: {e}).")

    print("\n   >>> Script 'run_rge_unification_suite.py' zakonczyl operation z najwyzsza precyzja! <<<")
    print("="*75)
